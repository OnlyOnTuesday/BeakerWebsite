/*
 * This file was not created by me (Michael Cooney)
 * I don't know who the original creator was
 */
...
export class CurrentStatus extends HTMLElement {
  ...
  async connectedCallback(){
    const info = await beaker.hyperdrive.getInfo("hyper://"+window.location.host)
    ...
    if(this.isOwner){
      ...
      this.peerEvents.addEventListener('join',e=>{
        ...
        this.statusTopic.send(e.peerId,new TextEncoder('utf-8').encode("Online"))
      })
      ...
      setInterval(()=>{
        this.peers.forEach((peer)=>{
          this.statusTopic.send(peer,new TextEncoder('utf-8').encode("Online"))
        })
      }, this.hearbeatInterval)

    } else {
      this.peerEvents.addEventListener('join',e=>{console.log(e.peerId)})
